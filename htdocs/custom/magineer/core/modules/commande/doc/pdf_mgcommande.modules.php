...
/**
 *	\file       htdocs/core/modules/commande/doc/pdf_mgcommande.modules.php
 *	\ingroup    commande
 *	\brief      File of Class to generate PDF orders with template mgcommande
 */
...
/**
 *	Class to generate PDF orders with template mgcommande
 */
class pdf_mgcommande extends ModelePDFCommandes
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "mgcommande";
		$this->description = $langs->trans('PDFmgcommandeDescription');
		...
		if (getMultidirOutput($object)) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('num')) {
						$this->printStdColumnContent($pdf, $curY, 'num', $i+1)	;
						$nexY = max($pdf->GetY(), $nexY);
					}
					// VAT Rate
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		$emetteur = empty($conf->global->MAIN_GENERATE_DOCUMENTS_SHOW_FOOT_DETAILS) ? '': $this->emetteur;

		return pdf_pagefoot($pdf, $outputlangs, 'ORDER_FREE_TEXT', $emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext, $this->page_largeur, $this->watermark);
	}
